...
test.describe('Lines disappear investigation', () => {
  ...
  test('Bio: continuous DOM monitoring for 10 seconds', async ({ page }) => {
    ...
    const finalLines = await page.$$('#lines-root div[style*="width: 1px"]');
  ...
  test('Bio: test with mouse movement', async ({ page }) => {
    ...
    // Initial check
    let lines = await page.$$('#lines-root div[style*="width: 1px"]');
    ...
    for (let i = 0; i < 10; i++) {
      ...
      lines = await page.$$('#lines-root div[style*="width: 1px"]');
